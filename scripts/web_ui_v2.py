...
# ── Import temporal progression systems ───────────────────────────────────
...
def build_world_info_markdown() -> str:
    ...
    # All locations in world — respect temporal gates
    ...
    for prefix in go_prefixes:
        if raw.startswith(prefix):
            ...
            if not dest_id:
                ...
                # TEMPORAL CHECK: Is location available in current phase?
                if session.story_phase and not session.story_phase.is_location_available(session.bundle, dest_id):
                    output = f"🔒 You cannot go to '{dest}' yet. The story hasn't reached that point."
                    session.history.append({"input": user_input, "output": output})
                    return output
    ...
    for prefix in talk_prefixes:
        if raw.startswith(prefix):
            ...
            if not char_id:
                ...
                # TEMPORAL CHECK: Is character available in current phase?
                if session.story_phase and not session.story_phase.is_character_available(session.bundle, char_id):
